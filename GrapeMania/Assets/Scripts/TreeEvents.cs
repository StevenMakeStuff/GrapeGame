...
public class TreeEvents : MonoBehaviour
{
    ...
    public IEnumerator FirstSpawnGrapes()
    {
        if(isWaveStarted == false)
        {
            ...
            if (player.score <= 11)
            {
                int g1Length = wave1.Length;

                grapeShooter.GetComponent<Animator>().enabled = true;

                for (int g1 = 0; g1 < g1Length; g1++)
                {
                    if (firstWave == true && g1 < wave1.Length && wave1[g1] != null)
                    {
                        Instantiate(wave1[g1], new Vector2(grapeShooter.transform.position.x, grapeShooter.transform.position.y), Quaternion.identity);
    ...
    public IEnumerator SecondSpawnGrapes()
    {
        if (isWaveStarted == false)
        {
            ...
            if (player.score <= 22)
            {
                ...
                int g2Length = wave2.Length;

                for (int g2 = 0; g2 < g2Length; g2++)
                {
                    if (secondWave == true && g2 < wave2.Length && wave2[g2] != null)
                    {
                        Instantiate(wave2[g2], new Vector2(grapeShooter.transform.position.x, grapeShooter.transform.position.y), Quaternion.identity);
    ...
    public IEnumerator ThirdSpawnGrapes()
    {
        if(isWaveStarted == false)
        {
            ...
            if (player.score <= 33)
            {
                int g3Length = wave1.Length;

                grapeShooter.GetComponent<Animator>().enabled = true;

                for (int g3 = 0; g3 < wave3.Length; g3++)
                {
                    if (thirdWave == true && g3 < g3Length && wave3[g3] != null)
                    {
                        Instantiate(wave3[g3], new Vector2(grapeShooter.transform.position.x, grapeShooter.transform.position.y), Quaternion.identity);
